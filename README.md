...
### 1. Оптимален для сценария "читатели vs писатели"
В блоге посты читают **гораздо чаще**, чем создают/обновляют:
- **95% запросов** (чтение популярных постов) — ответ за 1-5ms из Redis
- **5% запросов** (первые чтения или редкие посты) — ответ за 20-50ms из БД
